...
static char *
convert_number(char *result, char *buf, const char *bufend,
			   int base)
{
  int n = 0;
  int max = 1;
...
mark_subst_opts(struct subst *cmd)
{
  ...
  for (;;)
	...
	break;
      ...
      case 'w':
	cmd->outf = get_openfile(&file_write, "wb", true);
	return flags;
...
setup_label(struct sed_label *list, countT idx, char *name, const struct error_info *err_info)
{
      ...
      if (*p == '\\')
	{
	  ...
	  if (p == text_end)
	    ...
	    switch (*p)
	      ...
	      case '5': case '6': case '7': case '8': case '9': 
		tail->subst_id = *p - '0';
		if ((int)sub->max_id < tail->subst_id)
  ...
  for (;;)
	...
	case 'r':
	  ...
	  break;
        ...
        case 'R':
	  cur_cmd->x.fp = get_openfile(&file_read, "rb", false)->fp;
	  break;

        case 'W':
	case 'w':
	  cur_cmd->x.outf = get_openfile(&file_write, "wb", true);
	  break;
	...
	case 'y':
	  {
	    ...
	    dest_len = normalize_text(dest_buf, size_buffer (b2), TEXT_BUFFER);
            ...
            if (mb_cur_max > 1)
	      {
              ...
              {
	            unsigned char *translate = OB_MALLOC(&obs, YMAP_LENGTH, unsigned char);
