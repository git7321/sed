...
#define BAD_MODIF (NO_REGEX + sizeof("no previous regular expression"))
#define END_ERRORS (BAD_MODIF + sizeof("cannot specify modifiers on empty regexp"))

#ifdef REG_PERL
int
regncomp (regex_t *preg, const char *pattern, int length, int cflags)
{
...
compile_regex_1 (struct regex *new_regex, int needed_sub)
{
  ...
  if (needed_sub
      && (int)new_regex->pattern.re_nsub < needed_sub - 1
...
#ifdef REG_PERL
static void
copy_regs (struct re_registers *regs, regmatch_t *pmatch, int nregs)
{
  int i;
  ...
  else if (need_regs > (int)regs->num_regs)
    {
  ...
  for ( ; i < (int)regs->num_regs; ++i)
    regs->start[i] = regs->end[i] = -1;
}
#endif

int
match_regex(struct regex *regex, char *buf, size_t buflen, size_t buf_start_offset,
